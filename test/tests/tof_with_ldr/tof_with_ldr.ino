...
// Define the LED pin and LDR pin
#define LED_PIN 21
#define LDR_PIN 2 // Pin for the LDR
...
void loop() {
    ...
    if (millis() - startTime > firstEmailDelay) {
        if (abs(currentDistance - previousDistance) >= distanceThresholdCM && (millis() - lastEmailTime > emailCooldown)) {
            sendEmail(currentDistance, ldrValue);
...
void sendEmail(float distance, int ldrValue) {
